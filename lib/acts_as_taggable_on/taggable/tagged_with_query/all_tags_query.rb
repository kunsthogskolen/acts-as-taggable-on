      ...
      class AllTagsQuery < QueryBase
        ...
        def each_tag_in_list
          ...
          tag_list.each do |tag|
            ...
            arel_join = arel_join.join(tagging_alias)
                                 .on(on_conditions(tag, tagging_alias))
          end

          if options[:match_all].present?
            arel_join = arel_join.join(tagging_arel_table, Arel::Nodes::OuterJoin)
        ...
        def on_conditions(tag, tagging_alias)
          selects = if tag_translations_arel_table.present?
          ...
          if options[:start_at].present?
            on_condition = on_condition.and(
              tagging_alias[:created_at].gteq(options[:start_at])
            )
          end

          if options[:end_at].present?
            on_condition = on_condition.and(
              tagging_alias[:created_at].lteq(options[:end_at])
            )
          end

          if options[:on].present?
            on_condition = on_condition.and(
              tagging_alias[:context].eq(options[:on])
            )
          end

          if (owner = options[:owned_by]).present?
            on_condition = on_condition.and(
              tagging_alias[:tagger_id].eq(owner.id)
            ).and(
              tagging_alias[:tagger_type].eq(owner.class.base_class.to_s)
            )
          end
        ...
        def match_all_on_conditions
          on_condition = tagging_arel_table[:taggable_id].eq(
            taggable_arel_table[taggable_model.primary_key]
          ).and(
            tagging_arel_table[:taggable_type].eq(
              taggable_model.base_class.name
            )
          )

          if options[:start_at].present?
            on_condition = on_condition.and(
              tagging_arel_table[:created_at].gteq(options[:start_at])
            )
          end

          if options[:end_at].present?
            on_condition = on_condition.and(
              tagging_arel_table[:created_at].lteq(options[:end_at])
            )
          end

          if options[:on].present?
            on_condition = on_condition.and(
              tagging_arel_table[:context].eq(options[:on])
            )
          end
        ...
        def tags_that_matches_count
          ...
          taggable_model.find_by_sql(
            tag_arel_table.project(Arel.star.count)
                          .where(tags_match_type).to_sql
          )
        ...
        def order_conditions
          ...
          if options[:order_by_matching_tag_count].present? &&
             options[:match_all].blank?
            order_by << tagging_arel_table.project(
              tagging_arel_table[Arel.star].count.as('taggings_count')
            ).order('taggings_count DESC').to_sql
          end
        ...
        def tagging_alias(tag)
          ...
          adjust_taggings_alias(
            "#{alias_base_name[0..11]}_taggings_" \
            "#{ActsAsTaggableOn::Utils.sha_prefix(tag)}"
          )
        end
      end
    end
