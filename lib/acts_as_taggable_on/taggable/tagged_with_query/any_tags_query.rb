    ...
    module TaggedWithQuery
      class AnyTagsQuery < QueryBase
        ...
        def at_least_one_tag
          ...
          exists_contition =
            tagging_arel_table[:taggable_id].eq(
              taggable_arel_table[taggable_model.primary_key]
            ).and(
              tagging_arel_table[:taggable_type]
                .eq(taggable_model.base_class.name)
            ).and(
          ...
          if options[:start_at].present?
            exists_contition = exists_contition.and(
          ...
          if (owner = options[:owned_by]).present?
            exists_contition =
              exists_contition.and(
                tagging_arel_table[:tagger_id].eq(owner.id)
              ).and(
                tagging_arel_table[:tagger_type].eq(owner.class.base_class.to_s)
              )
          end
        ...
        def order_conditions
          ...
          if options[:order_by_matching_tag_count].present?
            order_by << "(SELECT count(*) FROM #{tagging_model.table_name} " \
                        "WHERE #{at_least_one_tag.to_sql}) desc"
          end
        ...
        def alias_name(tag_list)
          ...
          taggings_alias = adjust_taggings_alias(
            "#{alias_base_name[0..4]}#{taggings_context[0..6]}_" \
            "taggings_#{ActsAsTaggableOn::Utils.sha_prefix(tag_list.join('_'))}"
          )
